...
pub fn main() -> Result<(), anyhow::Error> {
    let (global_sub, _guard) = compact_file_logger(
        "examples/uart-int-overflow/uart-int-overflow.log",
        Level::TRACE,
    );
    ...
    let path = "examples/samples/uart-int-overflow/uart-int-overflow.elf";
    ...
    let policy = ttff::policy::TaintedOverflowPolicy::new_with(lang);
    ...
    let queue_corpus_path = PathBuf::from("examples/uart-int-overflow/queue");
    let crash_corpus_path = PathBuf::from("examples/uart-int-overflow/crashes");
